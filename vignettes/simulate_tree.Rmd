...
dat1 <- sim_adb_origin_samp(t_or, scale, shape, d, rho, origin_type, Xi_as, Xi_s)
...
shape <- c(100, 1)
scale <- av_lifetime/shape
dat2 <- sim_adb_origin_samp(t_or, scale,shape,d,rho,origin_type,Xi_as=Xi_as, Xi_s=Xi_s)

# plot with node/tip types
ggtree(dat1) + geom_rootedge() + geom_point(aes(x=x-branch.length, color=type)) + geom_tippoint(aes(color=type))
ggtree(dat2) + geom_rootedge() + geom_point(aes(x=x-branch.length, color=type)) + geom_tippoint(aes(color=type))
