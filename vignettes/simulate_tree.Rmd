...
microbenchmark(sim_adb_origin_complete(t_or, scale, shape, d))
```
```{r}
library(profvis)
...
p <- profvis({data <- sim_adb_origin_complete(t_or, scale, shape, d)})
p
```
...
microbenchmark(
  R_original = sim_adb_ntaxa_complete(ntaxa, scale, shape, d),
  Rcpp_fast  = sim_adb_ntaxa_complete_fast(ntaxa, scale, shape, d),
  times = 10
)
```

```{r}
# generating trees to validate the structure and ensure algorithm still works the same
data_cpp <- shape <- 100
av_lifetime <- 1
d <- 0.08
scale <- av_lifetime / shape
...
data_cpp <- sim_adb_ntaxa_complete_fast(ntaxa, scale, shape, d)
  
plot(data_cpp@phylo, show.tip.label=FALSE, root.edge=FALSE)
```
...
microbenchmark(
  R_original = sim_adb_origin_complete(t_or, scale, shape, d),
  Rcpp_fast  = sim_adb_origin_complete_fast(t_or, scale, shape, d),
  times = 10
)
```
